...
def measure_point(lockin: SR830, freq, n_tau, q):
    freq_used = lockin.set_internal_ref_cfg(freq=freq)[0]

    # Mantiene el factor de calidad ~constante
    integration_time = q / freq_used
    integration_time = lockin.set_filter_cfg(time=integration_time)[0]

    time.sleep(n_tau * integration_time)

    data, noise = promedio(lockin, tiempo=1)

    tau = integration_time
    scale = lockin.get_scale()

    return freq_used, data, noise, tau, scale


def anim_animate(n, ax, lines, lockin: SR830, df: pd.DataFrame):
    # n es el número del frame

    ax.set_ylim(df["Y [V]"].min(), df["Y [V]"].max())

    freq_used, data, noise, tau, scale = measure_point(lockin, freq)

    df["Frecuencia [Hz]"][n] = freq_used
    ...
    lines[0].set_data(freq_used, np.real(data))
    lines[1].set_data(freq_used, np.imag(data))

    return lines


def measure(lockin: SR830, freqs, n_tau=4, q=20) -> pd.DataFrame:
