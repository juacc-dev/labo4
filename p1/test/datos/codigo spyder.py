...
PATH = Path(
    r"C:\Users\publico\Documents\Laboratorio 4, 2025.2 - Grupo 6\datos 10-9\laton\forzado"
)

FREQ_MAX = 500       # Donde se corta el espectro (en Hz)
SKIP_SPECTRUM = 0    # Para ignorar los primeros puntos del espectro

N_PEAKS = 7          # Número de picos a encontrar
PEAK_SEP = 20        # Separación mínima entre picos (en Hz)
...
print(f"Archivo guardado en: {Path(*Path(filepath).parts[-4:])}")

#%%

# Para sacar datos del principio
...
y = (np.sin(35 * x) + np.sin(46 * x) + np.sin(45 * x))*np.exp(-x/100)
