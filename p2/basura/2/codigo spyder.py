...
def error_chi(freq, curr, volt, err_curr, err_volt, phase):
    ...
    return np.abs(err_real) + 1j * np.abs(err_imag)
...
def calculate_chi(df_v, df_i):
    ...
    err_volt = np.sqrt(df_v["Ruido X [V]"] ** 2 + sens_v ** 2 + (volt.real * 0.2 / 100) ** 2) + \
        1j * np.sqrt(df_v["Ruido Y [V]"] ** 2 + sens_v **
                     2 + (volt.imag * 0.2 / 100) ** 2)
...
filepath = PATH / f"({count}) con {MATERIAL} {GEOMETRIA} A-B.csv"
df_v.to_csv(filepath, index=False)
...
filepath = PATH / f"({count}) sin nada I.csv"
...
chi, chi_err = calculate_chi(df_v[::2], df_i)
...
K0 = RADIO[f"{MATERIAL} {GEOMETRIA}"] ** 2 / RHO[MATERIAL]
...
chi_fit = np.real(model(freq, *p_opt_real)) + 1j * \
    np.imag(model(freq), *p_err_imag)
